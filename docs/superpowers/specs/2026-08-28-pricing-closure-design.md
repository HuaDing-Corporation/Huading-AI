...
用户已在 2026-08-28 逐项确认：价格矩阵、租户覆盖范围、豆包与 CosyVoice 的差异化计价、统一价格策略方案、报价数据流、失败与并发语义、测试和生产门禁；并进一步确认豆包采用“用户付款先冻结 30000 → 平台负责人在豆包官方手工注册 → 管理员交付后结算或拒绝后解冻”的人工订单模型。平台官方自用音色与用户账号无关，保持原状。用户于 2026-08-29 复核并批准完整书面规格。
