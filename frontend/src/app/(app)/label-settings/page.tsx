...
import { LabelSettings } from "@/components/label/label-settings";
import { Sidebar } from "@/components/layout/sidebar";
import { TopBar } from "@/components/layout/top-bar";
import { copy } from "@/lib/copy";

/** 深度合成标识设置页（LABEL-UI-0001）—— 独立路由 /label-settings。外壳仿工作台。 */
      ...
      <div className="mx-auto grid max-w-[1600px] grid-cols-1 grid-rows-[auto_1fr] gap-5 md:grid-cols-[248px_minmax(0,1fr)]">
        ...
        <section className="flex min-w-0 flex-col gap-5">
          <nav className="flex items-center gap-1.5 px-1 text-[12.5px]" aria-label="面包屑">
            ...
            <span className="text-ink-soft">{copy.label.pageTitle}</span>
          </nav>

          <header className="px-1">
            <h1 className="text-[27px] font-semibold tracking-[1px] text-ink">{copy.label.pageTitle}</h1>
            <p className="mt-1 text-[13.5px] text-ink-soft">{copy.label.pageSubtitle}</p>
          </header>

          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(320px,520px)]">
            <LabelSettings />
          </div>
        </section>
      </div>
