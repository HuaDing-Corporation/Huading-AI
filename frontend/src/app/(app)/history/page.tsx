...
/** 图片历史统一模块页（HISTORY-UI-0001）—— 独立路由 /history，4 tab。外壳仿工作台其它独立页。 */
      ...
      <div className="mx-auto grid max-w-[1600px] grid-cols-1 grid-rows-[auto_1fr] gap-5 md:grid-cols-[248px_minmax(0,1fr)]">
        ...
        <section className="flex min-w-0 flex-col gap-5">
          <nav className="flex items-center gap-1.5 px-1 text-[12.5px]" aria-label="面包屑">
            ...
            <span className="text-ink-soft">{copy.historyImages.pageTitle}</span>
          </nav>

          <header className="px-1">
            <h1 className="text-[27px] font-semibold tracking-[1px] text-ink">{copy.historyImages.pageTitle}</h1>
            <p className="mt-1 text-[13.5px] text-ink-soft">{copy.historyImages.pageSubtitle}</p>
          </header>
