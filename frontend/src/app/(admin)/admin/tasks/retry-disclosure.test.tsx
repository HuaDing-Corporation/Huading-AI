...
// → 按量那条（task-f1）必须变红。⚠️ 模块级 mock 态：每用例重跑**不同**任务（f1/f3/f2 各一次），互不影响。
...
describe("任务重跑 · 回执披露三态（精确文案，不许静默扣费）", () => {
  it("按量估算（task-f1）→ 横幅精确：「已重新排队（task-f1），预计扣费约 1,501 积分，最终按实际成片时长结算」（alert 角色）", async () => {
    renderTasks();
    await retryTask("task-f1");
    const banner = await screen.findByRole("alert");
    expect(banner).toHaveTextContent(/^已重新排队（task-f1），预计扣费约 1,501 积分，最终按实际成片时长结算$/);
  });

  it("固定价实扣（task-f3）→ 横幅精确：「已重新排队（task-f3），将扣费 100 积分」（alert 角色）", async () => {
    renderTasks();
    await retryTask("task-f3");
    const banner = await screen.findByRole("alert");
    expect(banner).toHaveTextContent(/^已重新排队（task-f3），将扣费 100 积分$/);
  });

  it("不重复扣费（task-f2）→ 横幅精确：「已重新排队（task-f2），不会重复扣费」（status 角色，非 alert）", async () => {
    renderTasks();
    await retryTask("task-f2");
    const banner = await screen.findByText(/^已重新排队（task-f2），不会重复扣费$/);
