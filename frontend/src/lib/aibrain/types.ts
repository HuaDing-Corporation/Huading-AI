...
/**
 * 积分展示格式。默认最多 1 位小数、整数不带 `.0`（137.6256 → "137.6"，27.52 → "27.5"，20 → "20"）。
 *
 * 🔴🔴 **通用判据：格式化不得把一个非零的金额显示为零**（FIX5 · P1-2）。
 *    旧实现固定 `toFixed(1)` → `0.02` 变成 `"0"`，于是界面上出现「还差 **0** 积分」「需补齐 **0** 积分」
 *    —— 用户据此认为**不差**，而实际差一点，充值时就会充不够。这是方向性错误，比数字不精确严重得多。
 *    （BE 的金额精度允许这种正小数：钱包量化到 `Decimal("0.000001")`。）
 * 做法：从 1 位起**逐级提升精度**，直到显示出来不是零。1 位够用时行为与旧实现完全一致
 *    （所以 27.5 / 68.8 / 137.6 / 20 那些既有断言不受影响）。
 * ⚠️ 仍是四舍五入，故 `0.05 → "0.1"`：数值上夸大了一倍，但方向安全 ——「还差」类语义下宁可让用户
 *    多充一点，也不能让他以为不用充。**把非零说成零才是不可接受的那种错。**
 * ⚠️ 末行的 `0.000001`：BE 把积分量化到 1e-6，正常到不了那里；真到了（契约坏了 / 前端自算出极小值）
 *    也**不许回落到 "0"**，进位到最小可表示单位——同上，宁可夸大一个微不足道的量。
 *
 * 单点存在还有一个原因：让「同一个数在弹窗里、提示里、消息气泡里长得一样」。
 * 所有展示积分的地方都必须走这里（FIX5 把 `costLabel` 与钱包余额也并了进来）。
 */
export function formatCredits(credits: number): string {
  if (credits === 0) return "0";
  for (const digits of [1, 2, 3, 4, 5, 6]) {
    const text = credits.toFixed(digits);
    if (Number(text) !== 0) return digits === 1 ? text.replace(/\.0$/, "") : text;
  }
