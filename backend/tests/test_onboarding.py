...
from app.db.models import Asset, Subscription, TaskAsset, Tenant, UsageRecord, VideoTask, Voice
...
# ── P1-①: flush-phase IntegrityError (genuine race) → 409, not 500 ───────────


def test_concurrent_duplicate_slug_at_flush_returns_409(monkeypatch, auth_db, seed_plan) -> None:
    """A duplicate slug that slips past the pre-check (a real concurrent race)
    raises IntegrityError at db.flush() — which must surface as the unified-envelope
    409, not an unhandled 500. The pre-check only narrows the window; correctness
    must not depend on it. We simulate the race by forcing flush() to raise the same
    IntegrityError Postgres would on the tenants.slug unique violation (FIX2 P1-①).
    """
    real_flush = Session.flush

    def flush_conflict_on_tenant(self, *args, **kwargs):
        if any(isinstance(obj, Tenant) for obj in self.new):
    ...
    monkeypatch.setattr(Session, "flush", flush_conflict_on_tenant)

    # raise_server_exceptions=False so an unhandled exception comes back as a 500
    # response (the bug) instead of re-raising — lets us assert 409-not-500.
    ...
    # Transaction rolled back — no half-created tenant left behind.
    monkeypatch.undo()
    with auth_db() as db:
        assert db.scalar(select(Tenant).where(Tenant.slug == "race")) is None


# ── P1-②: email is per-tenant unique → same email, different slug both register ─


def test_same_email_different_slug_both_register(auth_db, seed_plan) -> None:
    """email is unique per-tenant (uq_users_tenant_email), NOT globally, so the same
    person's email may register under different tenant slugs — both must succeed.
    Guards the 'one person, many orgs' product rule against a regression toward a
    bogus global-email-unique / email_taken behavior (FIX2 P1-②)."""
    client = TestClient(app)
    first = client.post(
        "/api/v1/auth/register-tenant",
        json={
